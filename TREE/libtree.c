...
tree_print_recurse(struct fileinfo finfo)
{
  ...
  if (opts.dirsonly == 0 || S_ISDIR(finfo.st.st_mode)) {
  ...
  if (read_file_list(dirp, &file_list, &file_count) == -1) {
      ...
      goto exit; /* TODO: Handle EACCES (this was missing previously)*/
  }
  if (putchar('\n') == EOF) goto exit;
...
print_path_info(struct fileinfo finfo)
{
  char sep = '[';
  if (opts.perms) {
    char* mode = mode_string(finfo.st.st_mode);
    if (printf("%c%s", sep, mode) < 0) goto exit; /* TODO */
    sep = ' ';
  }
  if (opts.user) {
    /*  Hint: getpwuid(3) */
  ...
  if (opts.size) {
    /*  Hint: stat.h(0p) */
    intmax_t size = finfo.st.st_size;
    if (printf("%c%jd", sep, size) < 0) goto exit; /* TODO */
    sep = ' ';
  }
  if (sep != '[')
